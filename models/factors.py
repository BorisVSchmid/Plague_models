...
def run():
    span = [time.strftime("%Y-%m-%d") for time in pd.date_range(datetime(1990, 1, 1), datetime(2000, 12, 31))]
    print(span)
    # -- Params
    ...
    t = [x for x in range(1, len(decade)-1)]
    # - Human
    beta_h = .2
    s_h = 25000.
    i_h = np.zeros_like(t)
    ...
    plt.plot(rec_r, label="resistant rats")
    plt.legend(loc='best')
